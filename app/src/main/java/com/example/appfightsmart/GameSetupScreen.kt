...
fun GameSetupScreen(
    ...
    Box(Modifier.fillMaxSize()) {
        ...
        Column(
            ...
            SensorStatusRow(connected = sensorConnected, rssi = signalRssi, batteryPercent = batteryPercent, modifier = Modifier.padding(bottom = 8.dp, end = 4.dp))
            Text(stringResource(R.string.prepare_match).uppercase(), color = Color.White.copy(alpha = 0.82f), fontSize = 13.sp, fontWeight = FontWeight.Black, letterSpacing = 1.8.sp, modifier = Modifier.padding(top = 2.dp, bottom = 10.dp))
...
@Composable private fun SectionTitle(text: String) { Text(text.uppercase(), color = Color.White.copy(alpha = 0.76f), fontSize = 14.sp, fontWeight = FontWeight.Black, letterSpacing = 1.4.sp) }
@Composable private fun CounterSection(title: String, value: Int, rangeText: String, onMinus: () -> Unit, onPlus: () -> Unit) { Column(Modifier.fillMaxWidth()) { SectionTitle(title); Row(Modifier.fillMaxWidth().padding(top = 6.dp), horizontalArrangement = Arrangement.SpaceEvenly, verticalAlignment = Alignment.CenterVertically) { SmallMetalButton("−", onMinus); Text(value.toString(), color = Color.White, fontSize = 28.sp, fontWeight = FontWeight.Black, textAlign = TextAlign.Center, modifier = Modifier.width(80.dp)); SmallMetalButton("+", onPlus) }; Text(rangeText, color = Color.White.copy(alpha = 0.48f), fontSize = 12.sp, textAlign = TextAlign.Center, modifier = Modifier.fillMaxWidth()) } }
@Composable private fun SmallMetalButton(text: String, onClick: () -> Unit) { val shape = RoundedCornerShape(12.dp); Box(Modifier.width(72.dp).height(42.dp).clip(shape).background(Brush.linearGradient(listOf(Color(0xFF2A2D31), Color(0xFF60646A), Color(0xFF1A1C20)))).border(1.dp, Color.White.copy(alpha = 0.30f), shape).clickable(onClick = onClick), contentAlignment = Alignment.Center) { Text(text, color = Color.White, fontSize = 28.sp, fontWeight = FontWeight.Bold) } }
@Composable private fun MoveCategoryCard(text: String, selected: Boolean, modifier: Modifier = Modifier, onClick: () -> Unit) { val shape = RoundedCornerShape(14.dp); Card(modifier.clip(shape).clickable(onClick = onClick), shape = shape, colors = CardDefaults.cardColors(containerColor = Color.Transparent)) { Box(Modifier.fillMaxWidth().aspectRatio(3f)) { Image(painterResource(R.drawable.button), null, Modifier.matchParentSize().graphicsLayer(scaleX = 1.04f, scaleY = 1.18f), contentScale = ContentScale.FillBounds); if (!selected) Box(Modifier.matchParentSize().background(Color.Black.copy(alpha = 0.30f))); Box(Modifier.matchParentSize().border(if (selected) 2.dp else 1.dp, if (selected) Color.White else Color.LightGray.copy(alpha = 0.55f), shape), contentAlignment = Alignment.Center) { Text(text, color = Color.White, fontWeight = FontWeight.Black, fontSize = 17.sp, textAlign = TextAlign.Center) } } } }
@Composable private fun MoveOptionChip(text: String, selected: Boolean, modifier: Modifier = Modifier, onClick: () -> Unit) { val shape = RoundedCornerShape(18.dp); val colors = if (selected) listOf(Color(0xFF454545), Color(0xFF8B8B8B), Color(0xFFD8D8D8), Color(0xFF777777), Color(0xFF2F2F2F)) else listOf(Color(0xFF202225), Color(0xFF515151), Color(0xFF25272B)); Box(modifier.clip(shape).background(Brush.linearGradient(colors, Offset.Zero, Offset.Infinite), shape).border(if (selected) 2.dp else 1.dp, if (selected) Color.White.copy(alpha = 0.90f) else Color.LightGray.copy(alpha = 0.45f), shape).clickable(onClick = onClick).padding(vertical = 10.dp, horizontal = 8.dp), contentAlignment = Alignment.Center) { Text(text, color = Color.White, fontWeight = FontWeight.Black, fontSize = 14.sp, textAlign = TextAlign.Center, modifier = Modifier.fillMaxWidth()) } }
@Composable private fun SetupSummaryCard(players: Int, moves: Int, moveName: String, modifier: Modifier = Modifier) { SetupMetalPanel(modifier) { Row(Modifier.fillMaxWidth().padding(vertical = 14.dp), horizontalArrangement = Arrangement.SpaceEvenly, verticalAlignment = Alignment.CenterVertically) { SummaryCell(stringResource(R.string.players), players.toString()); SummaryDivider(); SummaryCell(stringResource(R.string.moves), moves.toString()); SummaryDivider(); SummaryCell(stringResource(R.string.types), moveName.uppercase()) } } }
@Composable private fun SummaryCell(label: String, value: String) { Column(horizontalAlignment = Alignment.CenterHorizontally) { Text(label.uppercase(), color = Color.White.copy(alpha = 0.55f), fontSize = 10.sp, fontWeight = FontWeight.Bold); Text(value, color = Color.White, fontSize = 18.sp, fontWeight = FontWeight.Black, maxLines = 1) } }
@Composable private fun SummaryDivider() { Box(Modifier.height(42.dp).width(1.dp).background(Color.White.copy(alpha = 0.18f))) }
@Composable private fun StartGameButton(enabled: Boolean, modifier: Modifier = Modifier, onClick: () -> Unit) { val shape = RoundedCornerShape(18.dp); Box(modifier.height(58.dp).clip(shape).background(Brush.horizontalGradient(if (enabled) listOf(Color(0xFF4A0808), Color(0xFFC62828), Color(0xFF4A0808)) else listOf(Color(0xFF252525), Color(0xFF3A3A3A), Color(0xFF252525)))).border(1.dp, if (enabled) Color(0xFFFF6A5E) else Color.White.copy(alpha = 0.22f), shape).clickable(enabled = enabled, onClick = onClick), contentAlignment = Alignment.Center) { Text(stringResource(R.string.start_game).uppercase(), color = Color.White.copy(alpha = if (enabled) 1f else 0.45f), fontSize = 23.sp, fontWeight = FontWeight.Black, letterSpacing = 2.sp) } }
